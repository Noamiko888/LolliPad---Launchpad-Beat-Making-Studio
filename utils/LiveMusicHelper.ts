...
// FIX: Changed type-only import to a regular import to comply with coding guidelines.
...
export class LiveMusicHelper extends EventTarget {
  ...
  public get activeStylePrompts() {
    return Array.from(this.prompts.values())
      .filter((p) => {
        return !p.promptId.startsWith('control-') && !this.filteredPrompts.has(p.text) && p.weight > 0;
      });
  }
  ...
  public readonly setWeightedPrompts = throttle(async (prompts: Map<string, Prompt>) => {
    ...
    if (this.activeStylePrompts.length === 0) {
